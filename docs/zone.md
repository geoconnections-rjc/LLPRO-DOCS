...
* **<span style="background:#FEF1E1; margin:1px;">&nbsp;HEATING MODE LOAD ENTRY&nbsp;</span>** Enter loads in the <span style="background:#FEF1E1; margin:1px;">&nbsp;HEATING COLUMN&nbsp;</span> to account for peak heating and annual ground loads for the current **Zone** according to the time block in which they occur.
    * The most critical values will be the peak loads placed in the time block in which they occur.
    * Enter the average <span style="background:#FEF1E1; margin:1px;">&nbsp;HEATING&nbsp;</span> loads in the remaining (three) time blocks.
* **<span style="background:#DFE8F1; margin:1px;">&nbsp;COOLING MODE LOAD ENTRY&nbsp;</span>** Enter loads in the <span style="background:#DFE8F1; margin:1px;">&nbsp;COOLING COLUMN&nbsp;</span> to account for peak cooling and annual ground loads for the current **Zone** according to the time block in which they occur.
    * The most critical values will be the peak loads placed in the time block in which they occur.
    * Enter the average <span style="background:#DFE8F1; margin:1px;">&nbsp;COOLING&nbsp;</span> loads in the remaining (three) time blocks. 
...
* **Total Cooling** The total amount of heat energy (kBtu or kWh) that must be removed from the space in cooling to maintain the space t-stat set point.
    * Enter the value for each respective month. **LLPRO** will use the annual sum and to calculate values for annual EFLH in cooling mode.
...
* **Peak Cooling** The peak cooling load for each month. **LLPRO** will find the month where peak cooling load occurs and use it for the design day analysis.
* **Total Heating** The total amount of heat energy (kBtu or kWh) that must be added to the space in heating to maintain the space t-stat set point.
    * Enter the value for each respective month. **LLPRO** will use the annual sum and to calculate values for annual EFLH in heating mode.
...
* **Peak Heating** The peak heating load for each month. **LLPRO** will find the month where peak heating load occurs and use it for the design day analysis.
...
* **Percentage of Peak** After importing monthly loads, you must specify the appropriate percentage (compared to the peak value) for the heating and cooling loads in each time block increment.
    * Be sure to enter the percentages according to the time block in which the loads occur. In multi-zone projects, **LLPRO** will use this information to determine the maximum heating and cooling loads for the GHEX design. 
    * The most critical values will be entered in the time block in which the peak values occur (100%).
    * Accuracy is not as critical for the percentage values in off-peak time blocks as they will simply be used to calculate the system part-load factor.
    * The recommended entries in the off-peak time blocks will be the percentage that corresponds with the **AVERAGE** heating and cooling loads in each respective time period.
...
* A properly formatted file would be saved in .CSV format and have 8760 lines of data in two columns (first column = <span style="background:#DFE8F1; margin:1px;">&nbsp;COOLING LOADS&nbsp;</span>, second column = <span style="background:#FEF1E1; margin:1px;">&nbsp;HEATING LOADS&nbsp;</span>).
...
* **<span style="background:#FEF1E1; margin:1px;">&nbsp;HEATING LOAD BY TIME BLOCK&nbsp;</span>** The specified heating loads (by time block) for the current **Zone**.
* **<span style="background:#DFE8F1; margin:1px;">&nbsp;COOLING LOAD BY TIME BLOCK&nbsp;</span>** The specified cooling loads (by time block) for the current **Zone**.
