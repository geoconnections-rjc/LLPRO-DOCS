...
* <span class="term">Quad U-Bend in Parallel</span> U-bend geometry described as having four parallel-connected u-bends installed in a single bore. With this selection, four u-bends form four separate flow paths by being individually connected to the header. **LoopLink PRO** accounts for the decrease in Reynolds number due to the parallel arrangement of the loops (resulting in lower gpm in each loop, possibly eliminating turbulence).
